defmodule ContractManagerWeb.CategoryController do
  ...
  def create(conn, %{"category" => category_params}) do
    with {:ok, %Category{} = category} <- Contracts.create_category(category_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "category" => category_params}) do
    ...
    with {:ok, %Category{} = category} <- Contracts.update_category(category, category_params) do
      render(conn, "show.json", category: category)
    end
  end
